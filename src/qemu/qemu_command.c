                             ...
                             char *sock_path)
{
    ...
    if (!sock_path) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                       _("Attempt to access non exist Unix domain socket."));
        return NULL;
    }

    chardev = g_strdup_printf("socket,id=chr-vu-%s,path=%s", alias, sock_path);

    return chardev;
}

/**
 * qemuBuildVhostuserDevStr
 * @buf: virBufferPtr to append the built string
 * @baseName: qemu vhost-user device basename string. Ex: vhost-user-blk for <blk>
 * @qemuCaps: virQEMUCapPtr
 * @devtype: virDomainDeviceType of the device. Ex: VIR_DOMAIN_DEVICE_TYPE_DISK
 * @devdata: *DefPtr of the device definition
 *
 * Build the qemu virtio -device name from the passed parameters. Currently
 * this is mostly about attaching the correct string prefix to @baseName for
 * the passed @type. So for @baseName "vhost-user-blk" and devdata->info.type
 * VIR_DOMAIN_DEVICE_ADDRESS_TYPE_PCI, generate "vhost-user-blk-pci"
 *
 * Returns: -1 on failure, 0 on success
 */
static int
qemuBuildVhostuserDevStr(virBufferPtr buf,
                         const char *baseName,
                         virDomainDeviceType devtype,
                         void *devdata)
{
                       ...
                       virQEMUCapsPtr qemuCaps)
{
    ...
    switch ((virDomainDiskBus) disk->bus) {
    case VIR_DOMAIN_DISK_BUS_IDE:
        if (disk->src->type == VIR_STORAGE_TYPE_VHOST){
            virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                           _("Setting vhost for ide disk is not supported "
                             "by this QEMU"));
            return NULL;
        }
    ...
    case VIR_DOMAIN_DISK_BUS_SCSI:
        if (disk->src->type == VIR_STORAGE_TYPE_VHOST){
            virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                           _("Setting vhost for scsi disk is not supported "
                            "by this Libvirt"));
            return NULL;
        }
        if (disk->device == VIR_DOMAIN_DISK_DEVICE_LUN) {
    ...
    case VIR_DOMAIN_DISK_BUS_VIRTIO:
        if(disk->src->type == VIR_STORAGE_TYPE_VHOST){
            if (qemuBuildVhostuserDevStr(&opt, "vhost-user-blk",
    ...
    case VIR_DOMAIN_DISK_BUS_USB:
        if (disk->src->type == VIR_STORAGE_TYPE_VHOST) {
            virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                           _("Setting vhost for usb disk is not supported "
                             "by this QEMU"));
            return NULL;
        }
                               ...
                               virQEMUCapsPtr qemuCaps)
{
    ...
    size_t i;
    if (disk->src->type == VIR_STORAGE_TYPE_VHOST) {
        if (!(chardev = qemuBuildVhostuserChardevStr(disk->info.alias,
                                 disk->src->path)))
            return -1;
                        ...
                        qemuDomainObjPrivatePtr priv)
{
    ...
    if (virQEMUCapsGet(qemuCaps, QEMU_CAPS_OVERCOMMIT)) {
        ...
        virCommandAddArgFormat(cmd, "mem-lock=%d",
                               ((0 <=def->mem.locked)&&(def->mem.locked<= 100)) ? def->mem.locked : 0);
