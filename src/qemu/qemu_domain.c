 ...
 * -drive for legacy setup ,the correct node name for -blockdev setups or
 * the correct id for -chardev setup.
 *
                              ...
                              char **backendAlias)
{
    *backendAlias = NULL;
    ...
    if (disk->src->type == VIR_STORAGE_TYPE_VHOST){
        *backendAlias = g_strdup_printf("chr-vu-%s", disk->info.alias);
        return 0;
    }
