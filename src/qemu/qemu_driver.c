                       ...
                       const char *xmlin, unsigned int flags)
{
   ...
   /* libvirt-domain.c already guaranteed these two flags are exclusive.  */
    if (flags & VIR_DOMAIN_SAVE_RUNNING)
        ...
        was_running = false;
   /* If the VIR_DOMAIN_PAUSED_AFTER_SAVE is set, we will set the vm to pause after save.  */
    if(flags & VIR_DOMAIN_PAUSED_AFTER_SAVE)
        paused_after_save = true;
    ...
    if (ret < 0)
        goto endjob;
    if(paused_after_save){
        VIR_INFO("Jorhson: To pause the vm(%s) after save", vm->def->name);
    }else{
        /* Shut it down */
                    ...
                    unsigned int flags)
{
    ...
    virCheckFlags(VIR_DOMAIN_SAVE_BYPASS_CACHE |
                  ...
                  VIR_DOMAIN_PAUSED_AFTER_SAVE, -1);
